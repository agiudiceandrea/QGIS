...
void TestQgsMapLayer::cloneServerProperties()
{
  ...
  // the clone must have received the source layer's server properties
  ...
  // and the source layer's own server properties must be unchanged
  QCOMPARE( source->serverProperties()->shortName(), u"short name"_s );
